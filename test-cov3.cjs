...
(async () => {
  ...
  for (const f of files) {
    ...
    let error = null;
    try {
      // Test: many random input patterns
      const rng = rngMod.makeRng(123);
      const inst = def.create(rng, { emit: () => {} });
      for (let i = 0; i < 500; i++) {
        const input = makeFakeInput();
        // Random 1-3 keys pressed
        if (i % 7 === 0) input.pressed.a = true;
        if (i % 11 === 0) input.pressed.b = true;
        if (i % 13 === 0) input.pressed.left = true;
        if (i % 17 === 0) input.pressed.right = true;
        if (i % 19 === 0) input.pressed.up = true;
        if (i % 23 === 0) input.pressed.down = true;
        if (i % 29 === 0) input.pressed.start = true;
        if (i % 31 === 0) input.pressed.select = true;
        inst.update(input);
        if (i % 30 === 0) inst.render(makeFakeCtx());
      }
    } catch (e) { error = e; }
